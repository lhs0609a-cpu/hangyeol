  ...
  return (
    ...
    >
      <Wrap>
        ...
        {/* ── 사진 셋 ──────────────────────────────────
            이 구간에만 사진이 여럿이다. 다른 구간은 한 장씩이고
            "두 장 이상 놓으면 사진첩이 된다" 가 이 파일의 규칙인데,
            여기서는 셋이 한 덩어리로 한 가지를 말한다 —
            한류가 건너간 통로가 셋이라는 것. 노래 · 놀이 · 그림이다.
            정사각으로 맞춰야 셋이 한 줄로 읽힌다. 비율이 제각각이면
            그때부터 사진첩이 된다.

            셋 다 남의 저작물이 아니고 사람도 찍히지 않았다.
            어느 공연인지 특정되지 않는 객석 · 과자와 도형 · 조선의 그림.
            스틸컷도 공연 사진도 멤버 사진도 로고도 쓸 수 없다 —
            13번 §5 가 경로를 전부 확인했고 하나도 없었다.
            캡션에 가수나 배우 이름을 붙이는 순간 셋 다 위반이 된다.

            셋째 장은 대역이 아니라 출처다. 그래서 가장 세다. */}
        <div
          style={{
            display: 'grid',
        ...
        >
          <PhotoFrame
            ...
            aspect="1 / 1"
            tone="ink"
            caption="노래 — 가사를 따라 부르려고 발음을 찾고, 발음을 찾다 문법에 닿습니다."
          />
          ...
          <PhotoFrame
            ...
            aspect="1 / 1"
            tone="ink"
            caption="놀이 — 화면 밖으로 나온 것은 장면이 아니라 낱말이었습니다. 달고나 · 딱지 · 무궁화꽃이 피었습니다."
          />

          <PhotoFrame
            src={photo('hojakdo').src}
            alt={photo('hojakdo').alt}
            width={photo('hojakdo').width}
            height={photo('hojakdo').height}
            aspect="1 / 1"
            focus="center top"
            tone="ink"
            caption="그림 — 조선의 민화입니다. 2025년 넷플릭스 역대 1위가 된 애니메이션의 호랑이와 까치가 여기서 나왔습니다."
          />
        </div>

        {/*
         * 셋째 장의 주장에만 출처가 필요하다.
         * 앞의 둘은 사진이 그 자체로 사실이지만("객석이다", "달고나다"),
         * 셋째는 "저 영화가 이 그림에서 왔다" 는 인과 진술이라서
         * 근거가 없으면 우리가 지어낸 말이 된다.
         */}
        <p
          style={{
            fontSize: 'var(--fs-caption)',
        ...
        >
          호작도(虎鵲圖) · 조선시대 작자 미상. 제작진이 민화를 참고했다고 밝힌 내용은{' '}
          <a
            href="https://en.wikipedia.org/wiki/KPop_Demon_Hunters"
            target="_blank"
            rel="noopener noreferrer"
            style={{ color: 'inherit' }}
          >
            위키백과가 Salon 기사를 출처로 정리해 두었습니다
          </a>
          .
        </p>
