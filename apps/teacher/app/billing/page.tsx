...
export default function BillingPage() {
  ...
  return (
    <Shell>
      ...
      <div className="mono" style={{ fontSize: 38, fontWeight: 500, letterSpacing: '-0.03em', marginTop: 8 }}>
        {won(data.total)}
      </div>
      ...
      {data.creditBalance > 0 && (
        <p className="mono" style={{ fontSize: 12, color: 'var(--jade)', marginTop: 6 }}>
      ...
      <Panel style={{ marginTop: 22 }}>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          {data.lines.length === 0 && data.waived.length === 0 && (
            <p style={{ fontSize: 13, color: 'var(--ink-4)', margin: '10px 0' }}>
              아직 청구할 주기가 없습니다
            </p>
          )}
        </div>
      ...
      <Panel style={{ marginTop: 14, background: 'var(--indigo-w)', border: '1px solid transparent' }}>
        <p style={{ margin: 0, fontSize: 12.5, color: 'var(--indigo)', lineHeight: 1.7 }}>
...
function Row({
  ...
  return (
    ...
    >
      ...
      <span style={{ flex: 1, fontSize: 11.5, color: 'var(--ink-4)' }}>{note}</span>
      <span
        className="mono"
        style={{ fontSize: 13, color: muted ? 'var(--chija)' : 'var(--ink)', whiteSpace: 'nowrap' }}
      >
        {right}
      </span>
    </div>
