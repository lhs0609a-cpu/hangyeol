...
export async function noteHome(studentId: bigint, now = new Date()): Promise<NoteHome> {
  ...
  const [due, activities, lastLesson] = await Promise.all([
    ...
    prisma.studentActivity.findMany({
      where: { studentId, kind: { notIn: ['learning_adjustment', 'learning_performance', 'teaching_observation'] }, occurredAt: { gte: addDays(now, -1) } },
      select: { kind: true },
    }),
...
async function weeklyStudyDays(studentId: bigint, now: Date): Promise<number> {
  const rows = await db().studentActivity.findMany({
    where: { studentId, kind: { notIn: ['learning_adjustment', 'learning_performance', 'teaching_observation'] }, occurredAt: { gte: addDays(now, -6) } },
...
async function streakDays(studentId: bigint, now: Date): Promise<number> {
  const rows = await db().studentActivity.findMany({
    where: { studentId, kind: { notIn: ['learning_adjustment', 'learning_performance', 'teaching_observation'] }, occurredAt: { gte: addDays(now, -60) } },
    select: { occurredAt: true },
