...
export const EXTERNAL_ASSETS: readonly ExternalAsset[] = Object.freeze([
  ...
  /*
   * 오징어 게임을 보이면서 넷플릭스 저작물을 쓰지 않는 방법.
   *
   * 스틸컷 · 포스터 · 캐릭터(영희 인형)는 전부 막혀 있다(아래 hallyu-ip-imagery).
   * 그런데 위키미디어 공용을 뒤져 보니 막히지 않은 것이 있었다 — 달고나다.
   *
   * 과자는 1960년대부터 있던 것이고, 찍힌 무늬는 도형 넷(○ △ ☆ ☂)이다.
   * 도형에는 저작권이 없고 놀이의 규칙에도 저작권이 없다. 사진의 저작권은
   * 촬영자에게 있는데 그 사람이 CC BY-SA 4.0 으로 풀어 놓았다.
   *
   * 이 저장소에서 Share-Alike 자산을 실제로 쓰는 첫 사례다.
   * 사진을 페이지에 싣는 것은 2차 저작물이 아니라 편집물이라 우리 코드에
   * 라이선스가 옮지 않는다. 다만 상호가 찍힌 아래쪽을 잘라 냈고, 자르는 것은
   * 변경이므로 우리가 내보내는 그 사진 자체는 같은 라이선스로 나간다.
   * /licenses 의 shareAlikeAssets() 가 이 사실을 화면에 밝힌다.
   */
  {
    id: 'dalgona-squid-game-shapes',
    name: '달고나 사진 (오징어 게임 도형)',
    kind: 'photo',
    license: 'CC-BY-SA-4.0',
    url: 'https://commons.wikimedia.org/wiki/File:Squid_Game_dalgona_cookies.jpg',
    attribution: '사진 Triplecaña (위키미디어 공용) · CC BY-SA 4.0 · 하단을 잘라 냄',
    usage: '랜딩 한류 구간. 넷플릭스 저작물이 아니라 과자와 도형이라 쓸 수 있다.',
    caveat:
      '이 사진이 통과하는 이유는 찍힌 것이 과자이기 때문이다. ' +
      '영희 인형 · 가면 · 초록 트레이닝복 · 로고는 넷플릭스의 저작물이라 같은 자리에 못 쓴다. ' +
      '캡션에 배우 이름을 붙이거나 작품 스틸인 것처럼 보이게 두면 그 순간 위반이 된다. ' +
      'Share-Alike 자산이므로 이 사진을 고쳐 쓰면 고친 것도 CC BY-SA 로 나간다.',
    verifiedAt: '2026-09-07',
  },

  /*
   * 케이팝 데몬 헌터스를 보이는 방법 — 대역이 아니라 출처를 보인다.
   *
   * 그 영화의 스틸컷 · 캐릭터 · 로고는 전부 막혀 있다(아래 hallyu-ip-imagery).
   * 로고는 저작권이 풀린 판본이 공용에 있지만 상표가 남아서 못 쓴다.
   *
   * 그런데 이 그림은 그 영화의 대역이 아니다. 영화에 나오는 호랑이와 까치가
   * 조선의 민화 호작도에서 왔다는 것은 제작진이 밝힌 사실이고, 그러므로
   * 이 그림은 그 영화의 출처다. 원본을 보이는 것은 복제가 아니다.
   *
   * 그림 자체는 조선시대 작자 미상이라 저작권이 소멸했고, 평면 저작물을
   * 그대로 찍은 복제물에는 새 저작권이 생기지 않는다(hunminjeongeum-haerye-scan 과 같은 근거).
   */
  {
    id: 'hojakdo-minhwa',
    name: '호작도 — 조선 민화 (호랑이와 까치)',
    kind: 'photo',
    license: 'PD-old',
    url: 'https://commons.wikimedia.org/wiki/File:Tiger_and_Magpie,_72x59.4_cm,_priviate_collection,_Japan.jpg',
    attribution: '호작도(虎鵲圖) · 조선시대 작자 미상 · 위키미디어 공용',
    usage: '랜딩 한류 구간. 케이팝 데몬 헌터스의 호랑이와 까치가 나온 원본.',
    caveat:
      '이 그림을 쓸 수 있는 이유는 조선의 민화이기 때문이지 그 영화와 관련이 있기 때문이 아니다. ' +
      '영화의 캐릭터 디자인 · 스틸컷 · 로고는 같은 자리에 못 쓴다. ' +
      '"저 영화가 이 그림에서 왔다" 는 인과 진술이므로 화면에 출처를 함께 적는다 — ' +
      '적지 않으면 우리가 지어낸 말이 된다.',
    verifiedAt: '2026-09-07',
  },
  ...
  },
  {
    ...
    caveat:
      '막히는 층이 둘이라서 하나가 풀려도 못 쓴다. ' +
      '(1) 저작권 — 스틸컷 · 포스터 · 공연 사진 · 로고는 넷플릭스와 소속사의 것이다. ' +
      '(2) 초상권 · 퍼블리시티권 — 출연자 개인의 권리이고 촬영자의 라이선스로 풀리지 않는다. ' +
      'BTS 멤버 사진은 위키미디어 공용에 실제로 있고 개중에는 자유 라이선스도 있다(2026-09-07 확인). ' +
      '그래도 쓰지 않는다 — 자유 라이선스는 촬영자의 저작권만 푼다. ' +
      '(3) 상표 — 케이팝 데몬 헌터스는 공용에 저작권이 풀린 로고 판본까지 있다. ' +
      '그것도 쓰지 않는다. 저작권이 풀려도 상표가 남고, 유료 상품의 랜딩에 남의 로고를 놓으면 ' +
      '제휴 관계로 읽힌다. 상표가 막는 것이 정확히 그 혼동이다. ' +
      '유료 상품의 랜딩에 사람 얼굴을 놓으면 그 사람이 상품을 보증하는 것처럼 읽히고, ' +
      '초상권이 막는 것이 정확히 그 용법이다. Pexels 항목에 적어 둔 제약과 같은 이유다. ' +
      '대신 이름과 사실은 출처와 함께 화면에 적는다 — 사실을 진술하는 것은 복제가 아니다. ' +
      '그림이 필요한 자리는 저작물도 사람도 찍히지 않은 사진으로 채운다 — ' +
      '어느 공연인지 특정되지 않는 객석(hallyu-lightsticks)과 달고나(dalgona-squid-game-shapes). ' +
      '그 사진들의 캡션에 가수나 배우 이름을 붙이면 그 순간 위반이 된다.',
    verifiedAt: '2026-09-07',
  },
  {
    id: 'piper-tts',
